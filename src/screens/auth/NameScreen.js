    ...
    try {
      ...
      if (response.assets && response.assets.length > 0) {
        ...
        if (serverResponse.status === 200) {
          ...
          console.log('Status is not 200 while uploading profile picture ', serverResponse.status);
        }
      } else {
  ...
  return (
    <SafeAreaView style={[styles.pdh16, { height: '100%' }]}>
      <View style={[{ top: '13%', justifyContent: 'left', alignItems: 'left' }]}>
        <Text style={[styles.mb8, { fontSize: 30, color: colors.black, textAlign: 'center' }, styles.fw400]}>
          Welcome to{' '}
          <Text style={[styles.mb8, { fontSize: 30, color: colors.black, textAlign: 'center' }, styles.fwBold]}>
            XSale
          </Text>
        </Text>
      </View>
      <View style={[{ top: '15%', alignItems: 'center', alignSelf: 'center' }]}>
        {profileUploading ? (
          ...
          <TouchableOpacity
            ...
            <Image
              source={profilePictureURL ? { uri: profilePictureURL } : icons.avatar}
              style={[styles.icon16, { height: 100, width: 100, resizeMode: 'cover' }]}
            />
          </TouchableOpacity>
        )}
      ...
      <View style={[{ top: '18%' }]}>
        ...
        {nameError && (
          <Text style={[{ color: colors.red }, styles.mt4, styles.mb24]}>
            Name must be a minimum of three characters. Please provide a valid name.
          </Text>
        )}
        <Button
          label={loading ? <ActivityIndicator size={'small'} color={colors.white} /> : 'Continue'}
          style={styles.mb24}
